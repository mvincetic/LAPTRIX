...
| B4 Formal visual checkpoint | Surface stabilization is complete at `b4b8f51`: all eight remote jobs pass. The regional landscape is integrated with pinned licensed historical terrain, protected foreground, original Blender vertex art, shared maps and source-scoped haze. Its full local gate (342 TypeScript / 178 Python), four byte-identical exports, 26 focused/broader and forty production-browser cases, 32 camera states and six native motion samples pass. Four real Play clips and eight matched before/after pairs are retained. Finish commit/push and remote verification, then clear the observed Onboard caption/cockpit overlap while preserving source access and controls. Keep full-circuit structures paused: terrain/forest remain simplified. The software-motion deadline remains unmet and documented. See REGIONAL_LANDSCAPE.md, VISUAL_CHECKPOINT.md and CI.md. |
