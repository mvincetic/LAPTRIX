...
| B2 Red Bull Ring GT vertical slice | Active: original Blender GT is integrated with lazy GLB delivery, native telemetry rig, independent reference materials and procedural fallback. Local gate, 64 pose checks, 13 Onboard states, six motion sequences and 38 production cases pass. Next: export/integrate the source-aligned start/finish–T1 environment, retune camera scale and judge the combined browser result before Formula work. |
