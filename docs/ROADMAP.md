...
| B2 Red Bull Ring GT vertical slice | Local acceptance passes: original Blender GT and source-aligned start/finish–T1 scenery, closer/lower Chase, lazy delivery and native fallbacks. Gate: 330 TypeScript / 162 Python, nine assets, three byte-identical exports, 17 targeted development and 39 production journeys. QA: 64 camera poses, six software motion sequences and an RTX 3060 Ti start-straight sample with 180 frames / 2.978 wall seconds / zero anchor drift. Verify the next remote run before Formula authoring. The previous GT run's stale daylight mesh-count assertion now checks actual surface coverage. |
