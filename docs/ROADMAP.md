...
| B2 Red Bull Ring GT vertical slice | Implemented at `df0a8ba`: 330 TypeScript / 162 Python, nine assets, three byte-identical exports, 17 targeted development and 39 production cases pass locally. QA passes 64 poses, 48 foliage/48 GT reference/45 terrain states and six hardware/software motion sequences. The pylon correction at `c698a90` passes remotely; that run exposes a stale 474-tree expectation that races the authored grandstand's exclusion of one crown. Correct and verify this final foliage fixture before declaring the slice's CI green. |
| B3 Generic Formula 2026 | Original editable source, native wheel rig, shaped aero surfaces and LAPTRIX livery are authored from public FIA direction and pass independent asset/reproducibility checks. Continue local browser integration and visual review while isolated B2 test corrections run remotely. Retain procedural fallback and native physics; stable promotion and milestone completion require the complete green gate. |
