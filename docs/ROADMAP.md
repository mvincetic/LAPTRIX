...
| B2 Red Bull Ring GT vertical slice | Implemented at `df0a8ba`: 330 TypeScript / 162 Python, nine assets, three byte-identical exports, 17 targeted development and 39 production cases pass locally. QA passes 64 poses, 48 foliage/48 GT reference/45 terrain states and six hardware/software motion sequences. Remote shard 5 exposed an all-GLB download counter in the older Dev Track test; its asset-specific correction passes the full gate, three development cases and production recovery. Verify the correction's remote run before Formula runtime promotion. |
| B3 Generic Formula 2026 | Prepare original Blender source while the isolated B2 QA correction runs remotely; the verified GT application remains unchanged. Author sculpted bodywork, halo, wings, native wheel rig and original LAPTRIX livery from public FIA design direction. Integrate only after the preceding remote gate and source validation succeed, retaining procedural fallback and native physics. |
