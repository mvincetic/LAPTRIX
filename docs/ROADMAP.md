...
| B4 Formal visual checkpoint | Five packed Blender ground maps serve road, runoff and terrain with exact geometry preservation and a 4.22 MB package. Local gates and native-GPU visual/motion QA pass. Remote `bab6b26` passes the full gate, four exports, forty production journeys and 181 development journeys, but daylight hits its per-test deadline and another shard leaves two cases unrun at its suite deadline. Run `b398f3f` verifies the daylight split and six-shard suite limits, but GPX page closure and the final apex project export hit their individual deadlines after successful assertions. Split saved/portable restoration and pixel/camera-playback journeys, retain every check and deadline, then verify remote CI before integrating the prepared regional landscape study. The separate local software motion deadline remains documented. Keep full-circuit structures paused. See SURFACE_MATERIALS.md, VISUAL_CHECKPOINT.md and CI.md. |
