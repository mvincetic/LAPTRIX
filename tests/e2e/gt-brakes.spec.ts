...
for (const [track, width] of [
  ...
  // Keep the full seek history in both journeys, but partition the expensive
  // camera matrix so CI setup and export still fit the existing minute budget.
  for (const [phase, cameraStart] of [
    ["coast/peak", 0],
    ["interpolation/return", 2],
  ] as const)
    test(`GT lamps preserve native demand during ${phase} camera changes on ${track}`, async ({
      page,
    }) => {
      ...
      const rise = current.samples.findIndex(
        (sample, i) =>
          i > 0 && sample.brake - current.samples[i - 1].brake > 0.2,
      );
      ...
      for (const [step, time] of [
        ...
        await expect
          .poll(async () =>
            (await lamps(page)).every((pair, i) =>
              pair.every(
                (lamp) => Math.abs(lamp.intensity - expected[i]) < 1e-9,
              ),
            ),
          )
          .toBe(true);
        ...
        if (step >= cameraStart && step < cameraStart + 2)
