...
for (const failure of ["unavailable", "malformed"] as const) {
  test(`optional authored foliage recovers from ${failure} delivery while preserving the paused engineering workspace`, async ({
    page,
  }) => {
    ...
    await page.route("**/*spruce*.glb*", (route) => {
      if (route.request().resourceType() === "fetch" && ++downloads === 1)
        return route.fulfill({
          status: failure === "unavailable" ? 503 : 200,
          body: failure === "unavailable" ? "Unavailable" : "Invalid GLB bytes",
        });
    ...
    page.on("response", (response) => {
      if (
        ...
        /\/spruce(?:-[a-zA-Z0-9_-]+)?\.glb/.test(response.url()) &&
        downloads > 1 &&
        response.ok()
      )
        ready = true;
    ...
    await expect
      .poll(() =>
        warnings.some((warning) =>
          warning.includes("Authored tree detail could not load"),
        ),
      )
      .toBe(true);
...
test("procedural foliage fallback recovers visibly while preserving the paused engineering workspace", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 1000 });
  const errors: string[] = [],
  ...
  await page.route("**/*spruce*.glb*", (route) =>
    route.request().resourceType() === "fetch"
      ? route.fulfill({
          status: 503,
          body: "Authored package unavailable throughout fallback recovery",
        })
      : route.continue(),
  );
  ...
  const missing = await canvas.screenshot({
    ...options,
    path: "artifacts/foliage-fallback-before.png",
  });
  for (let cycle = 0; cycle < 3; cycle++) {
    await page
      .getByRole("tab", { name: "Analysis Layers", exact: true })
  ...
  await canvas.screenshot({
    ...options,
    path: "artifacts/foliage-fallback-after.png",
  });
  expect(await project()).toEqual(before);
  expect(downloads).toBe(2);
  expect(solves).toBe(0);
