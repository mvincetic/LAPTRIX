...
test("the built Blender GT visibly replaces its fallback while preserving the paused workspace", async ({
  page,
}) => {
  ...
  let downloads = 0,
    delivered = false;
  await page.route("**/gt*.glb*", async (route) => {
  ...
  page.on("response", (response) => {
    if (
      ...
      /\/gt(?:-[\w-]+)?\.glb$/.test(new URL(response.url()).pathname) &&
      response.ok()
    )
      delivered = true;
  });
